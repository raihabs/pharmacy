...
<nav class="navbar col-lg-12 col-12 p-lg-0 fixed-top d-flex flex-row">
  <div class="navbar-menu-wrapper d-flex align-items-stretch justify-content-between">
    ...
    FROM product p LEFT JOIN archive a ON p.pr_id = a.pr_id WHERE (a.quantity <= 30 OR a.remarks != 'ACTIVE') AND a.date_updated_at IS NOT NULL AND p.notification != 'TRUE' ORDER BY  a.date_created_at DESC, a.date_updated_at DESC
LIMIT 70";
          ...
          $archive_count_list_result = mysqli_query($conn, $archive_count_list_query);
          $count = mysqli_fetch_array($archive_count_list_result);
          $notification__query = "SELECT COUNT(*), pr_id as pr_id FROM `notification` WHERE  `created_by` = '" . $_SESSION["user_id_admin"] . "'";
...
<a class="dropdown-item preview-item" onclick="updateNotification(<?php echo $row['pr_id']; ?>)" href="../admin/archive.php" style="text-decoration: none;">
                ...
                <?php
                    $notification_list_query = "SELECT * FROM `notification` WHERE `pr_id` = '" . $row["pr_id"] . "'  AND `created_by` = '" . $_SESSION["user_id_admin"] . "'";
                    ...
                    if (mysqli_num_rows($notification_list_result) == 0) { ?>
                      <button class="btn btn-primary mr-5 notif-button" style="margin-bottom: 50px; left:0; align-items: top; background: #024ab8; opacity: 25%; height: 120px; width: 180px; border: none; width: 400px; position: absolute;"></button>
                      <?php
                    ?>

                    <!-- <button class="btn mr-5 notif-button" onclick="updateNotification(<?php echo $row['pr_id']; ?>)" style="margin-bottom: 50px; left:0; align-items: top; background: <?php echo $color ?> !important; opacity: 25%; height: 120px; width: 180px; border: none; width: 400px; position: absolute;"></button> -->

                    <?php
                    } else
                    ?>
                <?php } else if (isset($_SESSION["user_id_inventory_clerk"])) { ?>
                  <a class="dropdown-item preview-item" href="../inventory_clerk/archive.php" style="text-decoration: none;">
                  <?php } ?>
            ...
            if (isset($_SESSION["user_id_admin"])) { ?>
              ...
              <?php } else if (isset($_SESSION["user_id_inventory_clerk"])) { ?>
                <a href="../inventory_clerk/archive.php">
                  <h6 class="p-3 mb-0">See all activity</h6>
                </a>
              <?php } ?>
