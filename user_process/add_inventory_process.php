...
$insert_inventory_sql = "INSERT INTO inventory (pr_id, item_code, sell_price, quantity,batch_no, lot_no, expiration_date, remarks, created_by,  updated_by, date_created_at, date_updated_at) 
                            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
...
$insert_archive_sql = "INSERT INTO archive (pr_id, item_code, sell_price, quantity, batch_no, lot_no,expiration_date, remarks, created_by, updated_by, date_created_at, date_updated_at) 
                            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                            ON DUPLICATE KEY UPDATE
                            sell_price=VALUES(sell_price), 
...
for ($i = 0; $i < count($item_codes); $i++) {
    ...
    $batch_no = $conn->real_escape_string($batch_nos[$i]);
    $lot_no = $conn->real_escape_string($lot_nos[$i]);
    ...
    if ($fetch_inventory_stmt->num_rows >  0) {
        ...
        $update_inventory_stmt->bind_param("dissiiissss", $sell_price, $new_quantity, $batch_no, $lot_no, $expiration_date, $remarks, $created_by, $user_id, $date_created_at, $date_updated_at, $item_code);
        ...
        // echo json_encode(array('status' => 400, 'msg' => $item_code . ' is already exist in the inventory table. ' ));
        //     exit();

    } else {
        // Insert the inventory table
        $insert_inventory_stmt->bind_param("isdiiissiiss", $pr_id, $item_code, $sell_price, $new_quantity, $batch_no, $lot_no, $expiration_date, $remarks, $user_id, $updated_insert_by, $date_updated_at, $date_insert_updated_at);
    ...
    if ($fetch_archive_stmt->num_rows >  0) {
        // Update inventory if the item exists
        $update_archive_stmt->bind_param("diiississss", $sell_price, $new_quantity, $batch_no, $lot_no, $expiration_date, $remarks, $created_by, $user_id, $date_created_at, $date_updated_at, $item_code);
        ...
        $insert_archive_stmt->bind_param("isdiiissiiss", $pr_id, $item_code, $sell_price, $new_quantity,  $batch_no, $lot_no,$expiration_date, $remarks, $user_id, $updated_insert_by, $date_updated_at, $date_insert_updated_at);
