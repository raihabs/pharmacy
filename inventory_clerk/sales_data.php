<table class="table" id="table">
    ...
    <tbody>
        ...
        $product_list_query = "SELECT s.sa_id, p.pr_id as pr_id, p.item_code as item_code, p.category,  p.image, s.sales_code,s.pr_id, s.sell_price, s.quantity, SUM(s.quantity) AS total_quantity,  s.total, SUM(s.total) AS total_orders, s.status , s.created_by, s.updated_by, s.date_created_at, s.date_updated_at
                        ...
                        FROM product p LEFT JOIN sale s ON p.pr_id = s.pr_id WHERE s.sales_code = '" . $product["sales_code"] . "' ORDER BY s.date_created_at DESC";
                                ...
                                if (mysqli_num_rows($item_list_result) > 0) {
                                    while ($list = mysqli_fetch_array($item_list_result)) { ?>
                                        ...
                                        <input type="hidden" name="quantity[]" value="<?= $list["quantity"] ? $list["quantity"] : '' ?>">
                                ...
                                <div class="row">
                                    <div class="col-6">
                                        ...
                                        if ($product['status'] == 'RELEASED') { ?>
                                            <button type="submit" class="btn btn-primary mr-2"> REFUND </button>
                                        <?php } else { ?>
