            ...
            <div class="main-panel">
                <div class="content-wrapper pb-0">
                    ...
                    <div class="page-header">
                        <h3 class="page-title">See All Report</h3>
                        ...
                        <div class="page-header flex-wrap">
                            ...
                            <div class="d-flex">
                                ...
                                <select id="sort_table" name="sort_table" class="btn btn-sm bg-white btn-icon-text border">
...
GROUP BY 
    YEAR(date_created_at)
                ...
                document.addEventListener('DOMContentLoaded', function() {
                    ...
                    // Trigger the function once to load the default view   
                    updateIframeSource();
                });


                function updateIframeSource1() {
                    const selectElement1 = document.getElementById('sort_table');
                    const iframeElement1 = document.getElementById('graphIframe');
                    const first_title = document.getElementById('first_title');
                    const selectedValue = selectElement1.value;

                    // PHP variables injected as JavaScript variables
                    const firstWeekEndDate = "<?php echo isset($first_week['end_date']) ? $first_week['end_date'] : ''; ?>";
                    const lastWeekEndDate = "<?php echo isset($last_week['end_date']) ? $last_week['end_date'] : ''; ?>";
                    const totalWeeklyTotal = "<?php echo isset($total_sales['total_weekly_total']) ? $total_sales['total_weekly_total'] : '0'; ?>";
                    const firstYearMonth = "<?php echo isset($first_year['month_year']) ? $first_year['month_year'] : ''; ?>";
                    const lastYearMonth = "<?php echo isset($last_year['month_year']) ? $last_year['month_year'] : ''; ?>";
                    const totalYearlyTotal = "<?php echo isset($total_sales_year['total_yearly_total']) ? $total_sales_year['total_yearly_total'] : '0'; ?>";

                    <?php

                    $product_query = "WITH total_orders_cte AS (
    SELECT SUM(s.total) AS total_orders_sum
    FROM sale s
    WHERE s.date_created_at >= DATE_SUB(CURDATE(), INTERVAL 3 MONTH)
)
SELECT 
    s.sa_id, 
    p.pr_id AS pr_id, 
    ...
    s.date_created_at, 
    s.date_updated_at,
    total_orders_cte.total_orders_sum
FROM product p 
LEFT JOIN sale s ON p.pr_id = s.pr_id 
JOIN total_orders_cte 
WHERE sa_id != '' 
AND s.date_created_at >= DATE_SUB(CURDATE(), INTERVAL 4 MONTH)
GROUP BY s.sales_code 
ORDER BY s.date_created_at DESC;
";
                    $product_result = mysqli_query($conn, $product_query);

                    if (mysqli_num_rows($product_result) > 0) {
                        while ($prd = mysqli_fetch_array($product_result)) {
                            $sales = $sales + $prd['total_orders'];
                        }
                    }
                    $ovrprd_query = "SELECT 
    p.pr_id AS pr_id, 
...
ORDER BY 
    i.pr_id DESC
";
                    $ovrprd_result = mysqli_query($conn, $ovrprd_query);

                    if (mysqli_num_rows($ovrprd_result) > 0) {
                        while ($ovrprd = mysqli_fetch_array($ovrprd_result)) {
                            $orders = $orders + $ovrprd['total_orders'];
                        }
                    }


                    ?>
                    // Change iframe source and content based on the selected option
                    if (selectedValue === 'Sale') {
                        iframeElement1.src = '../getdata/report_sales_data.php'; // Sale report URL
                        first_title.textContent = 'Sale Business Overview';
                        document.getElementById('title_date').textContent = `Show overview from ${firstWeekEndDate} - ${lastWeekEndDate} See Details`;
                        document.getElementById('date_paragraph').textContent = 'This report will return the sum of the total sales for each week in the past three months, including the current week.';
                        document.getElementById('total_num').textContent = `₱ <?php echo $sales ?>.00`;
                        document.getElementById('total_title').textContent = ' last 3 months ';
                    } else if (selectedValue === 'Inventory') {
                        iframeElement1.src = '../getdata/report_inventory_data.php'; // Inventory report URL
                        first_title.textContent = 'Inventory Overview';
                        document.getElementById('title_date').textContent = `Show overview from ${firstWeekEndDate} - ${lastWeekEndDate} See Details`;
                        document.getElementById('date_paragraph').textContent = 'This report will return the inventory data for each week in the past three months, including the current week.';
                        document.getElementById('total_num').textContent = '₱ ---'; // Adjust total if needed
                        document.getElementById('total_title').textContent = ' last 3 months ';
                    } else if (selectedValue === 'Overall') {
                        iframeElement1.src = '../getdata/report_overall_data.php'; // Yearly overview report URL
                        first_title.textContent = 'Overall Business Overview';
                        document.getElementById('title_date').textContent = `Show overview from ${firstWeekEndDate} - ${lastWeekEndDate} See Details`;
                        document.getElementById('date_paragraph').textContent = 'This report will return the sum of the total for each year in the past 1 year, including the current year.';
                        document.getElementById('total_num').textContent = `₱ <?php echo $orders; ?>.00`;
                        document.getElementById('total_title').textContent = ' last 3 months ';
                    }

                    // Set iframe dimensions
                    iframeElement1.style.width = '100%';
                    iframeElement1.style.height = '500px';
                    iframeElement1.style.border = 'none';
                    iframeElement1.style.margin = '0';
                    iframeElement1.style.overflow = 'hidden'; // Hide scrollbars
                }

                // Add event listener on page load
                document.addEventListener('DOMContentLoaded', function() {
                    const selectElement1 = document.getElementById('sort_table');
                    selectElement1.addEventListener('change', updateIframeSource1);
