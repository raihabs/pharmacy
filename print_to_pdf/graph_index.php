...
<body>
    ...
    $bar_query = "SELECT status, DATE_FORMAT(date_created_at, '%M') AS month_year, SUM(total) AS monthly_total FROM sale WHERE status = 'RELEASED' AND date_created_at >= DATE_SUB(CURDATE(), INTERVAL 6 MONTH) GROUP BY DATE_FORMAT(date_created_at, '%M') ORDER BY month_year DESC";
    $bar_result = mysqli_query($conn, $bar_query);

    $total_query = "SELECT status, DATE_FORMAT(date_created_at, '%Y-%m') AS month_year, SUM(total) AS monthly_total FROM sale WHERE status = 'RELEASED' AND date_created_at >= DATE_SUB(CURDATE(), INTERVAL 12 MONTH) GROUP BY DATE_FORMAT(date_created_at, '%Y-%m') ORDER BY month_year DESC";
