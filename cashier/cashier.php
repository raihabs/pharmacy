...
<body>
  ...
  <div class="container-scroller">
    ...
    <div class="container-fluid page-body-wrapper">
      ...
      <?php include "../include/cashier_main_panel.php"; ?>
...
status,
DATE_FORMAT(date_created_at, '%Y-%U') AS first_day,             -- Year and week number
...
ORDER BY  date_created_at ASC";
                  $first_day_result = mysqli_query($conn, $first_day_query);

                  $first_day = mysqli_fetch_array($first_day_result);

                  $last_day_query = "SELECT 
status,
DATE_FORMAT(date_created_at, '%Y-%U') AS last_day,             -- Year and week number
MIN(DATE_FORMAT(date_created_at, '%M %d, %Y')) AS start_date,    -- Start date of the week (Monday)
MAX(DATE_FORMAT(date_created_at, '%M %d, %Y')) AS end_date,      -- End date of the week (Sunday)
SUM(total) AS daily_total
FROM 
...
ORDER BY  date_created_at DESC";
                  $last_day_result = mysqli_query($conn, $last_day_query);

                  $last_day = mysqli_fetch_array($last_day_result);
      ...
      if (selectedValue === 'Daily') {
        iframeElement.src = '../getdata/report_data_daily_query_print.php';
        document.getElementById('first_title').textContent = 'Daily Business Overview';
        document.getElementById('title_date').textContent = 'Show overview from <?= $first_day['first_day'] ?> - <?= $last_day['last_day'] ?> See Details';
        document.getElementById('date_paragraph').textContent = 'This report will return the sum of the total for each week in the past 14 DAYs, including the current week.';
        document.getElementById('total_num').textContent = '₱ <?= $first_day['daily_total'] ? $last_day['daily_total'] : '0'  ?>.00';
        document.getElementById('total_title').textContent = ' last 14 days ';
      ...
      iframeElement.style.width = '50%'; // Set width as needed
