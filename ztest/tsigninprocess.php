...
if ($username == "" || $password == "") {
	...
	if (mysqli_num_rows($checkUser_run) > 0) {
		...
		if (mysqli_num_rows($checkPasswordrun) > 0) {
			...
			} else if ($row['role'] == 'Cashier' && $row['session_attempt'] <= 5) {
			...
			} else if ($row['role'] == 'Inventory Clerk' && $row['session_attempt'] <= 5) {
			...
			if ($check['session_attempt'] >= 5) {
				...
				$_SESSION['checkUser'] = $username;
				$response['success'] = "700";
				$response['message'] = "Attempt limit exceeded! Reset your Password. or Contact your Admin.";
			} else {
